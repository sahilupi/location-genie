...
export class MessagesComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    try {
      ...
      this.subscription.add(
        this._signalrClientService.messenger.subscribe(
          async (res: ChatMessage) => {
            const userRole = res.userRole === 3 ? 'user' : 'host';
            ...
            if (res) {
              ...
              if (
                ...
                userRole.toLowerCase() !== this.currenntRole.toLowerCase()
              )
              ...
              if (
                ...
                await this._signalrClientService.markMessagesAsRead(
                  [Number(res.id)],
                  this.currentUserId,
                  this.recieverId
                );
              ...
              if (
                ...
                const userIds = this.usersList.map((user) => user.userId);
      ...
      this.subscription.add(
        this._signalrClientService.onlineUsers.subscribe(
          // @ts-ignore
          (res: ChatOnlineUser[]) => {
      ...
      this.subscription.add(
        this._signalrClientService.seenMsgs.subscribe(
          // @ts-ignore
          async (msgId: number[]) => {
      ...
      this.subscription.add(
        this._signalrClientService.searchUsers.subscribe(
          // @ts-ignore
          (res: { data: ChatSearchedUser }) => {
            if (res.data) {
              if (this.usersList.some((user) => user.userId === res.data.id)) {
                this.snackbar.info('This user is already in your chat list');
              }
              if (
            ...
            if (res.data && res.data.length > 0) {
              ...
              if (this.isInitiatingUsers) {
                ...
                if (queryParams && queryParams['r'] && queryParams['s']) {
                  ...
                  this._signalrClientService.recieverId = this.recieverId;
      ...
      this.subscription.add(
        this._signalrClientService.chatHistory.subscribe(
          // @ts-ignore
          async (res: ChatMessage[]) => {
            if (res && res.length > 0) {
      ...
      this.subscription.add(
        this._signalrClientService.deletedMsgId.subscribe((id: number) => {
      ...
      this.subscription.add(
        this._signalrClientService.editedMsg.subscribe(
  ...
  private async initFunctions(): Promise<void> {
    this._signalrClientService.areNotificationsGlobal = false;
  ...
  private async getCurrentUser(): Promise<void> {
    ...
    this.currenntRole = userPayload.current_role;
  ...
  async getUsersByEmail(event: Event): Promise<void> {
    ...
    await this._signalrClientService.getUsersByEmail(email);
  }
  ...
  async addContactToChat(
    ...
    await this._signalrClientService.addContactToChat(
      currentUserId,
      recieverId
    );
    this.recieverId = recieverId;
    await this._signalrClientService.getContactList(currentUserId);
  ...
  async getChatHistory(
    ...
    await this._signalrClientService.getChatHistory(currentUserId, recieverId);
    const unreadMsgs = this.chatMessages.filter(
      (msg) => msg.isUnread && msg.senderId !== this.currentUserId
    );
    const foundUser = this.usersList.find(
  ...
  onSendMessagesWithFile(): void {
    ...
    this._signalrClientService.sendMessageWithFile(
      this.currentUserId,
      this.recieverId,
      '',
  ...
  async onDeleteMessage(
    ...
    this.index = index;
    await this._signalrClientService.deleteMessage(
      senderId,
      recieverId,
      messagedId
    );
  ...
  ngOnDestroy(): void {
    ...
    this._signalrClientService.areNotificationsGlobal = true;
    // await this._signalrClientService.revokeConnection(this.currentUserId);
  }
}
